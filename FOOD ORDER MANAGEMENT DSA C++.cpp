...
int main(){
	...
	node *temp;
                ...
                goto adminchoise;
                    ...
                    if(exist->data==fdata){
                    cls(); br(5);pre(3);  	cout<<" Food Serial Already Exist, Please Re-Enter  "; Sleep(2000);
                    goto foodserial;
                    }
                    fprice:
                    fflush(stdin);
                    ...
                    cin>>fprice;
					br(2);pre(4);  	cout<<"Submiting your data";for(int cs=0;cs<4;cs++){	cout<<" .";Sleep(500);}
	...
	while(temp->data != get_food_choice){
		temp = temp->next;
		if(temp==NULL){
			br(2); pre(2);  echo("Please Choose From List "); br(2); Sleep(1000);
			goto foodlist;
		}
	}
	if(get_food_choice == temp->data){
	 ...
	 cin>>fcquantity; cls();
		if(fcquantity==0){
			...
			cls(); middle1();pre(3); 	cout<<"Out of Stock ! "; Sleep(2000);
...
void main_menu(){
    ...
    cout<<"\t\t|             MAIN MENU            |"<<endl;
   	cout<<"\t\t------------------------------------\n"<<endl;
           pre(3); echo("===> 1. Item List"); Sleep(400);
	br(2); pre(3); echo("===> 2. Manager Control Room"); Sleep(400);
